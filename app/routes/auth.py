...
async def register_user(user_data: UserCreateSchema, db: AsyncIOMotorDatabase = Depends(get_db)):
    """
    ...
    if await collection.find_one({"email": user_data.email}):
        raise HTTPException(
            ...
            detail=f"This email is already registered as a {collection_name[:-1]}.",
        )
